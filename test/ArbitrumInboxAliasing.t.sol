...
contract ArbitrumInboxAliasingWithAA is Test {
    ...
    function test_DelegatedInboxCall() public {
        ...
        for (uint i = 0; i < logs.length; i++) {
            ...
            if (logs[i].topics[0] == keccak256("MessageDelivered(uint256,bytes32,address,uint8,address,bytes32,uint256,uint64)")) {
                // Decode the data to get the sender
                (, , address sender, , ,) 
                    = abi.decode(logs[i].data, (address, uint8, address, bytes32, uint256, uint64));
